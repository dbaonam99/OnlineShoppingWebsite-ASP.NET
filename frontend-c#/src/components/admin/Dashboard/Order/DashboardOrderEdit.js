...
export default function DashboardOrderCreate(props) {
    ...
    const [orderAvatar, setOrderAvatar] = useState("")
        ...
        axios.get(`https://localhost:4000/api/vietnam`)
            .then(res => {
                ...
                if (order) {  
                    ...
                    setOrderAvatar(order.orderAvatar)
                        ...
                        .then(res => {
                            ...
                            for (let i in order.orderList) {  
                                for (let j in products) {
                                    if (order.orderList[i].id === products[j].id) {
                                        products[j].count = order.orderList[i].amount
    ...
    const onSubmit = (event) => {
        ...
        axios.put(`https://localhost:4000/api/order/${order.id}`, {
            id: order.id,
            orderId: orderId,
            orderName: orderName,
