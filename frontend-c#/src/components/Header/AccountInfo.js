...
function AccountInfo(props) {
   ...
   const [userCreateDay, setUserCreateDay] = useState(null)
   ...
   useEffect(()=>{  
      if (userInfo) {
         ...
         setUserCreateDay(userInfo.userCreateDay)
         if (userInfo.userTinh !== "") {
            axios.get(`https://localhost:4000/api/vietnam`)
               .then(res => { 
         ...
         axios.get(`https://localhost:4000/api/order`)
            .then(res => {
               const orderList2 = []
   ...
   const submitInfo = (event) => {
      ...
      const url = `https://localhost:4000/api/user/UploadFile`;
      ...
      .then(res => {
         axios.put(`https://localhost:4000/api/user/${userInfo.id}`, {
            id: userInfo.id,
            userName: userName,
            userAvt: res.data || userAvt,
            userEmail: userEmail,
                        ...
                        <form onSubmit={submitInfo} encType="multipart/form-data" >
                           ...
                           <div className="create-box-row account-box-row flex">
                              ...
                              <div className="dashboard-right create-box-right"> 
                                 <input 
                                    onChange={(event) => { 
                                       setUserAvt(URL.createObjectURL(event.target.files[0])) 
                                    ...
                                    className="noborder"
                                    multiple="multiple"
                                    style={{height: '30px', width: 'max-content', marginBottom: '10px'}}
                                 ></input>
